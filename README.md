...
> [!IMPORTANT]
> This is a workstation environment, not a production one. No cluster here serves traffic, the hardened overlay is called `prod-shaped` on purpose, and [What this does not claim](#what-this-does-not-claim) is a section rather than a footnote.
...
| | |
|---|---|
| [From nothing to a storefront](docs/from-nothing.md) | **Start here if you have never run Magento.** A bare machine to a working store, and the errors you will actually meet |
...
| [How it is put together](docs/architecture.md) | What the runtimes share, the four hard problems, and the layout |
...
| **How many stores at once** | **as many as fit** | one; there is no second port for a second host |
...
>
> **This is about runtimes, not about stores.** Several stores coexist on Compose because they share the expensive tier and only one does background work at a time. That is a different arrangement with its own guard — see [More than one store](docs/sites.md).

## Requirements

- **Docker**, with BuildKit. That is the whole list for the Compose runtime.
...
- **`mkcert`**, and a certificate per hostname in `CERTS_HOME`. There is no wildcard shortcut; see the callout in [Getting started](docs/getting-started.md#quick-start) for why. If your setup needs a proxy reloaded after issuing, point `DEV_CERT_TOOL` at your own wrapper and it will be called instead.
...
ADR-003 and ADR-005 both end with a limitation rather than a resolution. That is the state of the work, not a gap in the writing.
...
- The specific failure modes worked through and written down: `ReadWriteMany` for a shared webroot, build-time compilation for an immutable image, cache invalidation through an edge that is not the ingress.
...
- **Multi-tenancy, which is not a gap — it is out of scope and stays that way.** Several stores share a data tier as a development convenience on one workstation. There is no isolation between them at the database, network or resource level, none is planned, and nothing here should grow toward it. Calling this tenancy would imply a security boundary that does not exist and was never wanted.
